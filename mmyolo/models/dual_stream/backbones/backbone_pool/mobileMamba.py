...
from mmyolo.models.dual_stream.backbones.backbone_pool.lib_mamba.vmambanew import SS2D
...
def build_mobile_mambe_stage(setting: list):
    stages , in_channels , out_channels , global_ratio , local_ratio, depth, kernels, down_ops , drop_path , ssm_ratio,forward_type = setting.stages, setting.in_channels, setting.out_channels,setting.global_ratio, setting.local_ratio, setting.depth, setting.kernels, setting.down_ops , setting.drop_path, setting.ssm_ratio, setting.forward_type
    in_channels = make_divisible(in_channels, setting.widen_factor)
    out_channels = make_divisible(out_channels, setting.widen_factor)
    depth = make_round(depth, setting.deepen_factor)
    dprs = [x.item() for x in torch.linspace(0, drop_path, depth)]
    stage = []
    for d in range(depth):
        stage.append(
            MobileMambaBlock(stages, in_channels, global_ratio, local_ratio, kernels, dprs[d], ssm_ratio=ssm_ratio,
                             forward_type=forward_type))
        if down_ops[0] == 'subsample':
            # Build MobileMamba downsample block
            # ('Subsample' stride)
            stage.append(torch.nn.Sequential(Residual(
                Conv2d_BN(in_channels, in_channels, 3, 1, 1, groups=in_channels)),
                Residual(FFN(in_channels, int(in_channels * 2))), ))
            stage.append(PatchMerging(in_channels , out_channels))
            stage.append(torch.nn.Sequential(Residual(
                Conv2d_BN(out_channels, out_channels, 3, 1, 1, groups=out_channels, )),
                Residual(
                    FFN(out_channels, int(out_channels * 2))), ))
